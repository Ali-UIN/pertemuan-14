...
Route::get('/test-accessor-scope', function () {
    ...
    $html .= '<h2>Buku: status_stok_badge</h2>';
    $bukus = Buku::all();
    $html .= '<ul>';
    foreach ($bukus as $buku) {
        $html .= '<li>' . $buku->judul . ' - ' . $buku->status_stok_badge . '</li>';
    }
    $html .= '</ul>';

    $html .= '<h2>Buku: tahun_label</h2>';
    $html .= '<ul>';
    foreach ($bukus as $buku) {
        $html .= '<li>' . $buku->judul . ' - ' . $buku->tahun_label . '</li>';
    }
    $html .= '</ul>';

    $html .= '<h2>Buku Terbaru (>= 2024)</h2>';
    ...
    $html .= '<h2>Buku Stok Menipis (< 5)</h2>';
    ...
    $html .= '<h2>Buku Harga Range (100000 - 180000)</h2>';
    $hargaRange = Buku::hargaRange(100000, 180000)->get();
    $html .= '<ul>';
    foreach ($hargaRange as $buku) {
        $html .= '<li>' . $buku->judul . ' - ' . $buku->harga_format . '</li>';
    }
    $html .= '</ul>';

    $html .= '<h2>Anggota: status_badge</h2>';
    $anggotas = Anggota::all();
    $html .= '<ul>';
    foreach ($anggotas as $anggota) {
        $html .= '<li>' . $anggota->nama . ' - ' . $anggota->status_badge . '</li>';
    }
    $html .= '</ul>';

    $html .= '<h2>Anggota: kategori_usia</h2>';
    $html .= '<ul>';
    foreach ($anggotas as $anggota) {
        $html .= '<li>' . $anggota->nama . ' - ' . $anggota->kategori_usia . '</li>';
    }
    $html .= '</ul>';
    ...
    foreach ($bulanIni as $anggota) {
        $html .= '<li>' . $anggota->nama . ' (' . $anggota->tanggal_daftar->format('d-m-Y') . ')</li>';
    }
    $html .= '</ul>';
